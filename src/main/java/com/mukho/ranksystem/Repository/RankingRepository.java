...
public interface RankingRepository extends CrudRepository<RankDto, Long> {
    ...
    @Query(value = "SELECT name, race, rating, wins, loses, ROUND(100. * wins / NULLIF(wins+loses, 0), 2) AS win_rate, " +
        "rank() OVER (ORDER BY rating DESC, 100. * wins/(wins+loses) DESC, wins DESC) AS rank_num " +
        "FROM gamer WHERE (wins+loses > 0) OR (wins + loses = 0 AND rating != 1000);", nativeQuery = true)
